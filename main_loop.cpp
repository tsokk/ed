...
bool command_s(const char **const ibufpp, int *const pflagsp,
  ...
  if (!sflags) {
    ...
    else if (**ibufpp == delimiter)
      ++*ibufpp; /* skip delimiter */
    if (!get_command_suffix(ibufpp, &pflags, &snum))
      return false;
    gmask = pflags & (GPR | GLS | GNP);
    if (gmask == 0)
      gmask = GPR;
  } else if (sflags & SGP)
    pflags ^= gmask;
...
int exec_command(const char **const ibufpp, const int prev_status,
  ...
  *ibufpp = skip_blanks(*ibufpp);
  ...
  case 'a':
    ...
    break;
  case 'c':
    if (!check_addr_range2(addr_cnt) || !get_command_suffix(ibufpp, &pflags, 0))
      return ERR;
    if (!isglobal)
      clear_undo_stack();
    if (!delete_lines(first_addr, second_addr, isglobal) ||
        !append_lines(ibufpp, current_addr(), current_addr() >= first_addr,
                      isglobal))
      return ERR;
    break;
  case 'd':
  ...
  case 'h':
  case 'H':
    if (unexpected_address(addr_cnt) || !get_command_suffix(ibufpp, &pflags, 0))
      return ERR;
    if (c == 'H')
      verbose = !verbose;
  ...
  case 'p':
    ...
    break;
  case 'm':
    if (!check_addr_range2(addr_cnt) || !get_third_addr(ibufpp, &addr))
      return ERR;
  ...
  case 'u':
    if (unexpected_address(addr_cnt) ||
        !get_command_suffix(ibufpp, &pflags, 0) || !undo(isglobal))
      return ERR;
    break;
  case 'w':
  case 'W':
  ...
  case '!':
    ...
    break;
  case '\n':
    if (!check_second_addr(current_addr() + (traditional() || !isglobal),
                           addr_cnt) ||
