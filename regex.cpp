...
regex_t *get_compiled_regex(const char **const ibufpp,
  ...
  if (test_delimiter && delimiter != **ibufpp) {
    set_error_msg("Missing pattern delimiter");
    return 0;
  }
  ...
  if (exp && exp != subst_regex_)
    ...
    exp = (&store[0] != subst_regex_) ? &store[0] : &store[1];
...
int replace_matched_text(char **txtbufp, int *const txtbufszp,
  ...
  for (i = 0; i < rlen; ++i) {
    ...
    if (rbuf[i] == '&') {
      int j = rm[0].rm_so;
      int k = rm[0].rm_eo;
      if (!resize_buffer(txtbufp, txtbufszp, offset + k - j))
        return -1;
      while (j < k)
        (*txtbufp)[offset++] = txt[j++];
    ...
    {
      if (!resize_buffer(txtbufp, txtbufszp, offset + 1))
        return -1;
  ...
  if (!regexec(subst_regex_, txt, se_max, rm, 0)) {
    ...
    do {
      if (global || snum == ++matchno) {
        changed = true;
        i = rm[0].rm_so;
        if (!resize_buffer(txtbufp, txtbufszp, offset + i))
          return -1;
        if (isbinary())
          newline_to_nul(txt, rm[0].rm_eo);
        memcpy(*txtbufp + offset, txt, i);
        offset += i;
    ...
    i = eot - txt;
    if (!resize_buffer(txtbufp, txtbufszp, offset + i + 2))
      return -1;
    if (isbinary())
      newline_to_nul(txt, i);
