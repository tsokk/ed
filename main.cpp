...
void show_help(void) {
  ...
  printf(
      ...
      "  -s, --quiet, --silent      suppress diagnostics, byte counts and '!' "
      "prompt\n"
...
void show_error(const char *const msg, const int errcode, const bool help) {
  if (msg && msg[0])
    fprintf(stderr, "%s: %s%s%s\n", program_name, msg,
            (errcode > 0) ? ": " : "", (errcode > 0) ? strerror(errcode) : "");
  if (help)
...
int main(const int argc, const char *const argv[]) {
  int argind;
  bool loose = false;
  const struct ap_Option options[] = {{'E', "extended-regexp", ap_no},
                                      {'G', "traditional", ap_no},
                                      {'h', "help", ap_no},
